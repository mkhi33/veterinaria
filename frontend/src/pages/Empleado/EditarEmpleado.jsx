...
function EditarEmpleado({ open, handleClose, empleado, handleGuardarEdicion }) {
    ...
    useEffect(() => {
        // Actualiza los datos en el estado cuando el empleado cambia
        if (empleado) {
            setNuevoEmpleado({
                ...
                numero_contratacion: empleado.numero_contratacion || '',
    ...
    useEffect(() => {
        // Cargar la lista de tipos de documento y puestos al abrir el modal
    ...
    const handleGuardar = async () => {
        if (validarInput()) {
            try {
                const response = await axios.put(`http://localhost:4000/api/empleados/${empleado.id}`, nuevoEmpleado);
